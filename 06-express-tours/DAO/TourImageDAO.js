...
exports.addTourImageIfNotExisted = async (tourId, imgName) => {
    ...
    const {request, insertFieldNamesStr,insertValuesStr} =
        dbUtils.getInsertQuery(TourImageSchema.schema, dbConfig.db.pool.request(), insertData);
    if (!insertFieldNamesStr || !insertValuesStr){
        throw new Error('Invalid insert param');
    }
    query += ' (' + insertFieldNamesStr + ') select ' + insertValuesStr +
        ` WHERE NOT EXISTS(SELECT * FROM ${TourImageSchema.schemaName} `+
        `WHERE ${TourImageSchema.schema.tourId.name} = @${TourImageSchema.schema.tourId.name} `+
        `AND ${TourImageSchema.schema.imgName.name} = @${TourImageSchema.schema.imgName.name})`;
    console.log(query);
